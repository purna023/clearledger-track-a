...
async function refresh() {
  ...
  const visible = rows.filter(r => !query || `${r.customer_name} ${r.customer_id} ${r.invoice_number}`.toLowerCase().includes(query));
...
async function submitImport(form) {
  ...
  try {
    ...
    const summary = `Imported ${result.imported}, skipped ${result.skipped}, rejected ${result.rejected}.`;
    const errors = result.errors?.map(e => `line ${e.line}: ${e.reason}`).join('; ');
    feedback.textContent = errors ? `${summary} ${errors}` : summary;
    await refresh();
  } catch (error) {
...
document.querySelector('#search').addEventListener('input', () => refresh().catch(e => { document.querySelector('#page-error').textContent = e.message; }));
