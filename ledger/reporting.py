...
def invoices(db, status='all'):
    ...
    for row in data:
        item = dict(row)
        item['amount'] = float(_money(item['amount']))
        item['paid'] = float(_money(item['paid']))
        item['balance'] = float(_money(Decimal(str(item['amount'])) - Decimal(str(item['paid']))))
        item['status'] = 'paid' if Decimal(str(item['balance'])) <= 0 else 'open'
        result.append(item)
    if status != 'all':
...
def overview(db):
    ...
    return {'invoices': rows, 'unmatched_payments': unmatched, 'summary': {
        ...
        'outstanding': float(_money(sum((max(0, Decimal(str(r['balance']))) for r in rows), Decimal('0')))),
    }}
